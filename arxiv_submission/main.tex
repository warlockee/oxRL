...
\title{Do Post-Training Algorithms Actually Differ?\\ Evidence from Mathematical Reasoning at Scale}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    ...
    On Qwen~2.5, SFT leads offline methods at $\leq$3B but collapses to worst at 7B, where DPO (83.4\%~$\pm$0.56) and SimPO (83.3\%~$\pm$1.79) dominate ($p < 0.001$ vs.\ SFT, $N{=}5$). On Gemma~3, the same pattern holds: SFT~$>$~DPO at 1B and 4B, but DPO~$\geq$~SFT at 12B (87.4\% vs.\ 86.7\%). DPO variant point-estimate rankings also reverse between 1.5B and 7B, though no individual comparison reaches significance at either scale---the variants are statistically unresolvable, making small-scale variant selection unreliable.
...
\textbf{Data.} All algorithms share the same data pipeline to eliminate data-source confounds. For each prompt in GSM8K's 7,473 training problems, we sample 16 responses from the target model at temperature 1.0 and score each with exact-match correctness. DPO/SimPO/IPO receive (chosen, rejected) pairs from the highest/lowest scoring responses. SFT trains on the \emph{chosen} trajectories from this same self-play dataset---not gold responses---ensuring that only the loss function differs between methods.
...
\textbf{Learning rate.} We sweep LR in $\{10^{-6}, 5{\times}10^{-6}, 10^{-5}, 5{\times}10^{-5}, 2{\times}10^{-4}\}$ for SFT and $\{10^{-6}, 5{\times}10^{-6}, 10^{-5}\}$ for DPO at 7B (Appendix~\ref{app:lr_ablation}). SFT's optimal LR is $10^{-5}$; rates above $5{\times}10^{-5}$ cause catastrophic forgetting. The ranking inversion persists under independently optimized hyperparameters for each algorithm.
...
\textbf{At 7B: point-estimate rankings reverse.} Table~\ref{tab:dpo_variant_7b} shows that the 1.5B ranking reverses. ORPO---top-2 at 1.5B---collapses to worst at 7B ($-$4.79~pp vs.\ DPO). EXO---below DPO at 1.5B---rises to the top ($+$1.00~pp). The 7B variant spread is 5.79~pp, larger than the DPO--SimPO gap.
...
\begin{table}[t]
\centering
\caption{Top 5 DPO variants at 7B (3 seeds each). Vanilla DPO: 83.38\% ($N{=}5$). Point-estimate rankings reverse: the 1.5B leader (ORPO) is the 7B laggard; the 1.5B underperformer (EXO) rises to top. No variant reaches Bonferroni-corrected significance at either scale.}
...
The practical significance is subtle: most real-world post-training starts from Instruct models, where algorithm choice \emph{does} produce measurable differences. But the Base ablations at both scales reveal these differences arise from how each loss function interacts with prior alignment, not from intrinsic properties of the objectives. Understanding the algorithm--initialization interaction appears more fruitful than designing new loss functions.
