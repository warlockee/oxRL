...
\textbf{(3)~Apparent differences largely reflect format compliance, not reasoning.} The 19.3~pp GSM8K spread collapses $36\times$ to 0.54~pp on MATH and $41\times$ to 0.47~pp on general-domain benchmarks.
Together, these findings imply that small-scale algorithm selection---the dominant paradigm in both research and practice---is unreliable, and that practitioners should validate at deployment scale rather than extrapolating from cheap proxies.
\end{abstract}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    ...
    \item \textbf{Format compliance, not reasoning, drives the spread.}
    The 19.3~pp GSM8K spread collapses $36\times$ to 0.54~pp on competition-level MATH and $41\times$ to 0.47~pp on general-domain benchmarks. Algorithms differ primarily in how quickly they acquire the target output format; reasoning improvements are a secondary component.
\end{enumerate}
...
\subsection{Finding 3: Format Compliance Drives the Spread}
\label{sec:format_compliance}

Table~\ref{tab:math_results} compares GSM8K and MATH results at 1.5B. The 19.3~pp GSM8K spread collapses to \textbf{0.54~pp} on MATH---a $36\times$ compression. Rankings shift: SGRPO drops from 1st to 4th, SimPO rises from last to 2nd. On general-domain benchmarks (ARC-C, HellaSwag, WinoGrande), the spread collapses further to \textbf{0.47~pp} ($41\times$; Appendix~\ref{app:figures}).

At 1.5B, three lines of evidence converge: (1)~the cross-task compression, (2)~flexible extraction compressing the 3B spread from 11.5~pp to 6.6~pp (Appendix~\ref{app:format}), and (3)~the near-zero general-domain spread. However, this picture changes at scale. At Gemma~12B, the MATH spread \emph{increases} to 18.7~pp (DPO 50.2\% vs.\ SimPO 32.1\%), and at Qwen~14B the spread reaches 17.0~pp (SimPO 41.0\% vs.\ SFT 24.0\%). At 14B, general benchmarks show $<$1.2~pp spread (Table~\ref{tab:14b_results}), confirming that format compliance still dominates on GSM8K---but MATH reveals genuine reasoning differences that emerge at larger scales.
...
\begin{enumerate}[leftmargin=*,itemsep=1pt]
    \item \textbf{Validate at deployment scale.} Rankings at $\leq$3B do not predict 7B behavior---for both core algorithms and DPO variants. This is the paper's most important practical message.
    \item \textbf{Use DPO as the default.} DPO ranks near the top at both 1.5B and 7B in Qwen, and overtakes SFT at 12B in Gemma. It has the lowest seed variance ($\sigma = 0.56$ at 7B) and requires no special infrastructure.
...
In decreasing order of leverage: model scale (${\sim}$50~pp), initialization (${\sim}$15~pp), training paradigm (SFT vs.\ preference: ${\sim}$6~pp at 7B), and loss function choice within the preference family ($<$1~pp for DPO vs.\ SimPO; up to 5.8~pp across DPO variants at 7B).
However, loss function choice \emph{across} variants is scale-dependent: the 7B variant spread (5.8~pp from ORPO to EXO) exceeds the training paradigm gap at 1.5B.
...
\midrule
\textbf{Total training} & $\sim$302 & -- & -- & $\sim$\textbf{485} \\[2pt]
Evaluation (all checkpoints) & $\sim$302 & 5--30 min & 1 & $\sim$85 \\[2pt]
\midrule
\textbf{Grand total} & & & & $\sim$\textbf{570 GPU-h} \\
\bottomrule
\end{tabular}
\end{table}

At $\sim$\$3/GPU-hour, the total cost is approximately \$1,710---modest given the breadth (302 training runs across 2 model families, 7 scales) and substantially less than training a single 7B model from scratch.
