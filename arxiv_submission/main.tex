...
In a controlled study spanning ${\sim}$290 training runs across two model families (Qwen~2.5 0.5B--14B, Gemma~3 1B--12B), 8 algorithms, 20 DPO variants, and up to $N{=}5$ seeds, we find three results that challenge common intuitions.
\textbf{(1)~Rankings invert with scale.} At $\leq$1.5B, SFT is the strongest offline method; at 7B, SFT collapses to worst while DPO leads ($p < 0.001$, $N{=}5$). The same inversion appears in Gemma~3 (1B--12B) and strengthens at Qwen~14B, where DPO leads SFT by 2.3~pp. DPO variant rankings also fully invert between 1.5B and 7B.
...
These findings are based on ${\sim}$290 training runs on Qwen~2.5 (0.5B--14B), supplemented by 27 runs on Gemma~3 (1B--12B), spanning 8 algorithms, 20 DPO variants, 4 evaluation domains (GSM8K, MATH, ARC-C/HellaSwag/WinoGrande), and up to $N{=}5$ seeds with formal statistical testing.
...
\begin{table}[t]
\centering
\caption{Qwen 2.5-14B results on GSM8K (\%) and MATH (\%). 3 seeds each; LoRA. The inversion strengthens: DPO leads GSM8K by 2.3~pp over SFT, but SimPO dominates MATH. General benchmarks (ARC-C, HellaSwag, WinoGrande) show $<$1.2~pp spread.}
\label{tab:14b_results}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Algorithm} & \textbf{GSM8K Mean} & \textbf{GSM8K Seeds} & \textbf{MATH} & \textbf{General Avg} \\
\midrule
Base & 79.53 & --- & 16.8 & 74.2 \\
SFT & 84.53$_{\pm 0.19}$ & 84.3, 84.5, 84.8 & 24.0 & 73.8 \\
DPO & \textbf{86.78}$_{\pm 0.23}$ & 87.1, 86.6, 86.7 & 25.8 & 73.9 \\
SimPO & 86.48$_{\pm 0.68}$ & 85.6, 86.6, 87.3 & \textbf{41.0} & 73.7 \\
\midrule
\textit{GSM8K spread} & \multicolumn{2}{c}{\textit{2.3~pp (DPO$-$SFT)}} & \textit{17.0~pp} & \textit{$<$1.2~pp} \\
\bottomrule
\end{tabular}
\end{table}
...
SimPO & 1.1 & 39.3$_{\pm 0.8}$ & 32.7$_{\pm 1.5}$ & & \textbf{39.1}$_{\pm 2.2}$ \\
\midrule
\textit{Spread} & \textit{15.5} & \textit{8.1} & \textit{17.5} & & \textit{16.1} \\
\bottomrule
\end{tabular}
\end{table}
...
Post-training algorithm rankings invert with scale: the method that wins at 1.5B loses at 7B, a finding replicated across Qwen~2.5 (up to 14B) and Gemma~3 (up to 12B) and confirmed with formal statistical testing ($p < 0.001$, $N{=}5$). The inversion strengthens with scale: the DPO--SFT gap grows from ${\sim}$0.5~pp at 12B to 2.3~pp at 14B. Initialization---not the loss function---explains most algorithmic differences (3--15$\times$ spread compression when starting from Base). At small scale, residual differences largely reflect format compliance ($36\times$ spread compression from GSM8K to MATH at 1.5B), but at large scale MATH reveals genuine reasoning differences that are both architecture- and algorithm-dependent.
