...
In a controlled study spanning ${\sim}$308 training runs across two model families (Qwen~2.5 0.5B--14B, Gemma~3 1B--12B), 8 algorithms, 20 DPO variants, and up to $N{=}5$ seeds---using model-specific self-play data matching standard practitioner workflows---we find three results that challenge common intuitions.
...
\textbf{(1)~Rankings invert with scale (from Instruct checkpoints).} At $\leq$1.5B, SFT is the strongest offline method; at 7B, SFT collapses to worst while DPO leads ($p < 0.001$, $N{=}5$). The same inversion appears in Gemma~3 (1B--12B) and persists at Qwen~14B (DPO leads SFT by 2.2~pp), though the within-family gap narrows from 6.0~pp at 7B. DPO variant rankings are also statistically unresolvable at both scales, rendering small-scale variant selection unreliable.
...
These findings are based on ${\sim}$281 training runs on Qwen~2.5 (0.5B--14B), supplemented by 27 runs on Gemma~3 (1B--12B), spanning 8 algorithms, 20 DPO variants, 4 evaluation domains (GSM8K, MATH, ARC-C/HellaSwag/WinoGrande), and up to $N{=}5$ seeds with formal statistical testing.
...
\textbf{Qwen 2.5 (primary).} Qwen 2.5~\citep{qwen2025qwen25} Instruct at 0.5B, 1.5B, 3B, 7B, and 14B. Our experiments comprise: (1) core comparison of 8 algorithms across 4 scales ($\sim$70 runs, $N{=}5$ seeds for core methods at 7B), with a full-FT/LoRA factorial at both 3B and 7B; (2) DPO variant taxonomy at 1.5B (20 variants $\times$ 5 seeds = 100 runs) and a stratified subset of 5 at 7B (15 runs); (3) 14B deployment-scale validation with SFT, DPO, SimPO ($N{=}3$ seeds each); (4) ablations ($\sim$57 runs), including Base-vs-Instruct initialization and hyperparameter sensitivity; (5) a fixed-dataset control training 1.5B on 14B self-play data (6 runs, Section~\ref{sec:fixed_dataset}). Total: $\sim$281 training runs, $\sim$685 GPU-hours on H100s (Appendix~\ref{app:compute}).
...
At \textbf{3B}, severe format non-compliance produces a floor effect (base model: 62.9\% flexible but 10.7\% strict-match), making algorithmic comparisons unreliable at this scale. Under flexible extraction, the 3B base model solves 62.9\% of problems---comparable to 1.5B Instruct---but post-training degrades this (SFT: 44.3\%, Appendix~\ref{app:3b_flex}). Because 3B uses full fine-tuning with the same LR ($10^{-6}$) validated at 1.5B and 7B, this collapse may reflect hyperparameter sensitivity specific to this intermediate scale (a higher capacity model under full-parameter updates). We cannot rule out that a scale-specific LR sweep would recover performance; our 7B LR ablation (Appendix~\ref{app:lr_ablation}) shows that even moderate LR increases cause large accuracy drops, suggesting that 3B full-FT may be particularly sensitive. We therefore treat the 3B results as a caveat rather than a finding, and our core claims rest on the 1.5B, 7B, 12B, and 14B comparisons.
...
Critically, this pattern is \emph{architecture-dependent}. At Qwen~14B, \textbf{SimPO} dominates MATH (39.1\%~$\pm$2.2) over DPO (25.8\%~$\pm$0.4) by +13~pp---the reverse of Gemma. Meanwhile, general benchmarks (multiple-choice; see Appendix~\ref{app:general_benchmarks}) show $<$2~pp spread at all scales and both architectures (Table~\ref{tab:14b_results}), consistent with format-driven differences on those evaluations. The two-regime picture is: at small scale, algorithms differ shallowly (format); at large scale, algorithms differ deeply (reasoning), but \emph{which algorithm wins depends on architecture and task}. Both regimes make small-scale extrapolation unreliable.
...
Table~\ref{tab:fixed_dataset} reports the results (3 seeds each). \textbf{SFT still beats DPO by +5.1~pp} ($58.38\% \pm 0.49$ vs.\ $53.32\% \pm 0.69$), confirming that model scale---not data quality---is the primary driver of the ranking inversion.
...
DPO & 52.62 & 54.28 & 53.07 & 53.32 & 0.69 \\
\bottomrule
\end{tabular}
...
The 3B collapse is a legitimate concern: if a single LR ($10^{-6}$) causes catastrophic failure at one scale, perhaps the inversion at 1.5B and 7B is also an artifact of that LR happening to favor different algorithms at different scales. Three pieces of evidence argue against this. First, the LR ablation at 7B (Appendix~\ref{app:lr_ablation}) shows that $10^{-6}$ is independently optimal for \emph{both} SFT and DPO---DPO does not benefit from a different LR than SFT, ruling out the possibility that $10^{-6}$ selectively favors DPO at 7B. Second, the 3B failure is \emph{non-selective}: it degrades all algorithms similarly (SFT: 18.4\%, DPO: 14.4\%, SimPO: 6.9\%---all catastrophic), consistent with a scale-specific optimization pathology rather than an algorithm-specific hyperparameter interaction. Third, the inversion replicates across Qwen and Gemma with the same LR, and these two families have different architectures, pretraining data, and optimization landscapes. An LR artifact that produces the same inversion pattern in both families would require a remarkable coincidence. We nonetheless acknowledge that scale-specific LR sweeps at every scale point would strengthen the evidence (see Limitations).
...
\midrule
\textbf{Total training} & $\sim$308 & -- & -- & $\sim$\textbf{685} \\[2pt]
Evaluation (GSM8K, MATH, general) & $\sim$400+ & 5--600 min & 1 & $\sim$200 \\[2pt]
\midrule
\textbf{Grand total} & & & & $\sim$\textbf{885 GPU-h} \\
\bottomrule
\end{tabular}
\end{table}

The total compute ($\sim$885 GPU-hours) is modest given the breadth ($\sim$308 training runs across 2 model families, 8 scales, 4 evaluation domains) and substantially less than training a single 7B model from scratch.
