...
\textbf{(1)~Rankings invert with scale (from Instruct checkpoints).} At $\leq$1.5B, Self-Play SFT---which trains on the model's own correct trajectories, functionally rejection sampling fine-tuning (RFT)---is the strongest offline method; at 7B, Self-Play SFT collapses to worst while DPO leads ($p < 0.001$, $N{=}5$). A gold-data control confirms this is not an RFT artifact: standard SFT on MetaMathQA also dominates at 1.5B but converges to DPO at 7B. The same inversion appears in Gemma~3 (1B--12B) and persists at Qwen~14B (DPO leads SFT by 2.2~pp), though the within-family gap narrows from 6.0~pp at 7B. DPO variant rankings are also statistically unresolvable at both scales under shared default hyperparameters ($\beta{=}0.1$, LR${=}10^{-6}$); this tests plug-and-play transferability rather than per-variant-tuned capability ceilings, but it renders small-scale variant selection unreliable in the practitioner workflow we evaluate.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{main_result.png}
\caption{\textbf{Summary of findings.} Across $\sim$350 controlled training runs spanning Qwen~2.5 (0.5B--14B) and Gemma~3 (1B--12B), three patterns emerge.
\textbf{(A)} On Qwen 2.5 GSM8K, Self-Play SFT (SP-RFT) leads at $\leq$1.5B but collapses at 7B, where DPO ($83.4\%\pm0.56$) and SimPO ($83.3\%\pm1.79$) dominate ($p<0.001$ vs.\ SP-RFT). The same SP-RFT $\to$ DPO inversion replicates on Gemma~3 between 1B (SP-RFT $26.7\%>$ DPO $21.5\%$) and 12B (DPO $87.4\%>$ SP-RFT $86.7\%$).
\textbf{(B)} Replacing the Instruct checkpoint with Base compresses the inter-algorithm spread from 15.7~pp to 1.07~pp at 1.5B, and from 6.00~pp to 1.82~pp at 7B---initialization, not the loss function, accounts for most apparent algorithm differences.
\textbf{(C)} At small scale (1.5B), the 19.3~pp GSM8K spread collapses to 0.54~pp on MATH ($36\times$); at large scale, MATH reveals genuine reasoning differences but they are architecture-dependent (DPO best on Gemma~12B, SimPO best on Qwen~14B).
Together: small-scale algorithm selection is anti-predictive for mathematical reasoning---validate at deployment scale.}
\label{fig:summary}
\end{figure}
...
\textbf{(1)~SFT;} \textbf{(2)~Online RL} (PPO, and GRPO with three loss variants: SGRPO, GSPO, CISPO);
...
\textbf{GRPO loss variants.} The three GRPO variants differ in how the importance ratio between current and behavior policies is aggregated and clipped, which controls the credit-assignment granularity and the stability of on-policy updates:
\textbf{SGRPO} applies a token-level clipped surrogate---the per-token importance ratio is clipped to $[1{-}\epsilon_{\text{low}}, 1{+}\epsilon_{\text{high}}]$ before being multiplied by the group-normalized advantage, equivalent to PPO's policy loss without a critic. This is our default for dense models and serves as the GRPO representative throughout the paper. \textbf{GSPO} aggregates the importance ratio at the sequence level (one ratio per response) rather than per token; this absorbs per-token log-probability drift between the rollout engine (vLLM) and the trainer (HF/DeepSpeed), making it preferable for MoE models where expert-routing divergence amplifies token-level noise. \textbf{CISPO} uses the clipped ratio as a detached weight applied to the log-probability rather than as a multiplicative factor, yielding a more conservative update that we include as a stability fallback. Unless otherwise noted, ``online RL'' results in the main tables refer to SGRPO; GSPO and CISPO are reported as ablations in Appendix~\ref{app:scaling}.
...
\textbf{SGRPO setup.} As defined in Section~\ref{sec:framework}, SGRPO is the token-level clipped-surrogate variant of GRPO~\citep{shao2024deepseekmath}: for each prompt we sample a group of $n{=}4$ responses from the current policy with vLLM, score each via exact-match correctness on GSM8K, compute group-normalized advantages, and update the policy with a per-token importance-ratio clip $\epsilon = 0.2$ (no critic, no value head). The reference KL coefficient is $\lambda_{\text{KL}} = 0$ by default following the DeepSeekMath recipe; a small KL ablation is provided in Appendix~\ref{app:kl_ablation}. SGRPO (online RL) leads at 1.5B ($58.0\% \pm 0.57$, +3.6~pp over SFT) at ${\sim}6\times$ the compute cost.
