...
In a controlled study spanning ${\sim}$275 training runs across two model families (Qwen~2.5 0.5B--7B, Gemma~3 1B--12B), 8 algorithms, 20 DPO variants, and up to $N{=}5$ seeds, we find three results that challenge common intuitions.
\textbf{(1)~Rankings invert with scale.} At $\leq$1.5B, SFT is the strongest offline method; at 7B, SFT collapses to worst while DPO leads ($p < 0.001$, $N{=}5$). The same inversion appears in Gemma~3: SFT dominates at 1B and 4B, but DPO overtakes SFT at 12B. DPO variant rankings also fully invert between 1.5B and 7B.
...
These findings are based on ${\sim}$275 training runs on Qwen~2.5 (0.5B--7B), supplemented by 27 runs on Gemma~3 (1B--12B), spanning 8 algorithms, 20 DPO variants, 3 evaluation domains, and up to $N{=}5$ seeds with formal statistical testing.
...
\textbf{Qwen 2.5 (primary).} Qwen 2.5~\citep{qwen2025qwen25} Instruct at 0.5B, 1.5B, 3B, 7B. Our experiments comprise: (1) core comparison of 8 algorithms across 4 scales ($\sim$70 runs, $N{=}5$ seeds for core methods at 7B), with a 3B full-FT/LoRA factorial; (2) DPO variant taxonomy at 1.5B (20 variants $\times$ 5 seeds = 100 runs) and top 5 at 7B (15 runs); (3) ablations ($\sim$57 runs), including Base-vs-Instruct initialization and hyperparameter sensitivity. Total: $\sim$275 training runs, $\sim$490 GPU-hours on H100s (Appendix~\ref{app:compute}).
...
A 2$\times$2 factorial (Table~\ref{tab:lora_2x2}) shows that the massive 3B$\to$7B accuracy jumps (DPO: +67.3~pp, SFT: +59.9~pp) with LoRA held constant confirm that \textbf{scale}, not LoRA, drives the inversion. An LR ablation (Appendix~\ref{app:lr_ablation}) further rules out the LR confound.
...
\begin{table}[t]
\centering
\caption{2$\times$2 factorial disentangling scale from LoRA (Qwen 2.5). The 3B$\to$7B gains (DPO: +67.4~pp, SimPO: +76.4~pp) with LoRA held constant confirm that \textbf{scale} drives the inversion. 7B values: 5-seed means $\pm \sigma$.}
\label{tab:lora_2x2}
\small
\begin{tabular}{@{}llcc@{}}
\toprule
...
Three lines of evidence converge: (1)~the cross-task compression, (2)~flexible extraction compressing the 3B spread from 11.5~pp to 6.6~pp (Appendix~\ref{app:format}), and (3)~the near-zero general-domain spread. Algorithms differ primarily in how quickly they acquire the target output format; reasoning improvements are secondary.
...
Post-training algorithm rankings invert with scale: the method that wins at 1.5B loses at 7B, a finding replicated across Qwen~2.5 and Gemma~3 and confirmed with formal statistical testing ($p < 0.001$, $N{=}5$). Initialization---not the loss function---explains most algorithmic differences (3--15$\times$ spread compression when starting from Base). The residual differences largely reflect format compliance efficiency, not reasoning ability ($36\times$ spread compression from GSM8K to MATH).
...
\textbf{Limitations.}
Our study uses two model families (Qwen~2.5, Gemma~3) up to 12B on mathematical reasoning. Generalization to instruction following, safety, or creative writing remains open. Qwen experiments use synthetic preference data. The Gemma validation has 2 TBD seeds that will be added upon completion. We use LoRA at the largest scales; full FT at 7B+ is beyond our compute budget but would strengthen the conclusions. All code, configs, and raw outputs are released for independent verification.\footnote{Code and data available upon acceptance.}
