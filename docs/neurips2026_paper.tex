...
In a controlled study spanning ${\sim}$344 training runs across two model families (Qwen~2.5 0.5B--14B, Gemma~3 1B--12B), 8 algorithms, 20 DPO variants, and up to $N{=}5$ seeds---using model-specific self-play data matching standard practitioner workflows---we find three results that challenge common intuitions.
...
These findings are based on ${\sim}$317 training runs on Qwen~2.5 (0.5B--14B), supplemented by 27 runs on Gemma~3 (1B--12B), spanning 8 algorithms, 20 DPO variants, 4 evaluation domains (GSM8K, MATH, ARC-C/HellaSwag/WinoGrande), and up to $N{=}5$ seeds with formal statistical testing.
...
\textbf{Qwen 2.5 (primary).} Qwen 2.5~\citep{qwen2025qwen25} Instruct at 0.5B, 1.5B, 3B, 7B, and 14B. Our experiments comprise: (1) core comparison of 8 algorithms across 4 scales ($\sim$70 runs, $N{=}5$ seeds for core methods at 7B), with a full-FT/LoRA factorial at both 3B and 7B; (2) DPO variant taxonomy at 1.5B (20 variants $\times$ 5 seeds = 100 runs) and a stratified subset of 5 at 7B (15 runs); (3) 14B deployment-scale validation with SFT, DPO, SimPO ($N{=}3$ seeds each); (4) ablations ($\sim$57 runs), including Base-vs-Instruct initialization and hyperparameter sensitivity; (5) a fixed-dataset control training 1.5B on 14B self-play data (6 runs, Section~\ref{sec:fixed_dataset}); (6) a 1.5B LR sweep for SFT and DPO (18 runs, Appendix~\ref{app:lr_ablation_15b}); (7) a 3B LR sweep for SFT and DPO (18 runs, Appendix~\ref{app:lr_ablation_3b}). Total: $\sim$317 training runs, $\sim$700 GPU-hours on H100s (Appendix~\ref{app:compute}).
...
Because training data is self-generated at each scale, dataset quality correlates with model size. The fixed-dataset control (Section~\ref{sec:fixed_dataset}) directly tests this: training 1.5B on 14B-generated data does not flip the SFT--DPO ranking (SFT leads by 5.1~pp, comparable to the 5.3~pp gap with own data). Both algorithms benefit comparably from higher-quality data (SFT: +4.0~pp, DPO: +4.2~pp), leaving the gap stable. Three converging lines of evidence now support the conclusion that the inversion is scale-driven: (1)~the fixed-dataset control holds data quality constant and preserves the ranking; (2)~the Base-vs-Instruct ablation (Section~\ref{sec:base_ablation}) shows that \emph{initialization} accounts for 3--15$\times$ more variance than the algorithm itself; (3)~the inversion replicates across Qwen and Gemma, two families with different pretraining data and formatting conventions. We note that this paper targets the Datasets \& Benchmarks track: its contribution is a controlled experimental framework and reproducible empirical findings under the practitioner workflow that dominates real-world post-training~\citep{shao2024deepseekmath, deepseek2025r1}, not a theoretical claim about intrinsic loss function properties.
...
\midrule
\textbf{Total training} & $\sim$344 & -- & -- & $\sim$\textbf{700} \\[2pt]
Evaluation (GSM8K, MATH, general) & $\sim$420+ & 5--600 min & 1 & $\sim$210 \\[2pt]
\midrule
\textbf{Grand total} & & & & $\sim$\textbf{910 GPU-h} \\
\bottomrule
\end{tabular}
\end{table}

The total compute ($\sim$910 GPU-hours) is modest given the breadth ($\sim$344 training runs across 2 model families, 8 scales, 4 evaluation domains) and substantially less than training a single 7B model from scratch.
