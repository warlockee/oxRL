...
The Base-vs-Instruct ablation (Section~\ref{sec:base_ablation}) identifies one mechanism: at small scale the Instruct model rarely produces correct solutions, so RFT on the few successful trajectories yields large gains while preference methods waste capacity modeling pairs of incorrect outputs; at 7B the Instruct model already solves most GSM8K problems and RFT hits a ceiling, while DPO's contrastive signal still extracts gradient from incorrect trajectories. The gold-data SFT control (Section~\ref{sec:controls}) shows even standard SFT on MetaMathQA converges to DPO at 7B ($83.32$ vs.\ $83.38$) despite dominating at 1.5B by $+12$~pp, so the inversion is not merely a self-play data-saturation artifact but a genuine property of how supervised vs.\ contrastive objectives interact with model scale.
...
Four design choices interact: the fixed LR $10^{-6}$ is below the standard LoRA recommendation but an ablation rules out under-training (Appendix~\ref{app:lr_ablation}); SGRPO was trained without explicit KL but adding $\lambda_\mathrm{KL}{=}0.01$ yields $+0.15$~pp (Appendix~\ref{app:kl_ablation}); the 7B full-FT factorial (Appendix~\ref{app:lora_factorial}) confirms the inversion under both regimes; and the effective batch size of 8 prompts is conservative for DPO, which already wins at 7B regardless. Online RL ablations (SGRPO, GSPO, CISPO) are in Appendix~\ref{app:online_rl}.
...
\textbf{Limitations.}
Two families (Qwen~2.5 14B, Gemma~3 12B) on math; the MC-only general-benchmark control limits transfer to generative tasks. The fixed-dataset control is asymmetric (no converse 7B-on-1.5B-data run). 0.5B is at the protocol's noise floor (Appendix~\ref{app:seed_invariance}); 3B Qwen Instruct has a known LR pathology (Appendix~\ref{app:lr_ablation_3b}). The 20-variant DPO sweep uses shared $\beta{=}0.1$, LR$=10^{-6}$. Code, configs, per-seed JSONs release upon acceptance.
