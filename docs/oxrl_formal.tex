...
Given that small-scale rankings are anti-predictive, what should a practitioner do when target-scale validation is too expensive? We turn the inversion into actionable guidance using \emph{rank stability across scales} as the selection criterion. Defining a rank-stability score $S(a)$ as the maximum drop in cross-scale rank position relative to 1.5B (lower is better), Tables~\ref{tab:main_results}--\ref{tab:cross_arch} give \sprft{} $1{\to}4{\to}3$ ($S{=}3$), DPO $4{\to}1{\to}1$ ($S{=}0$), SimPO $5{\to}2{\to}2$ ($S{=}0$); the same monotone pattern holds on Gemma~3. Because the inversion is one-way (preference methods overtake SFT/RFT-style), \textbf{DPO is the rank-stable bet}---never worse than 4th, best from 7B onward across both architectures. Empirically the DPO--\sprft{} gap turns positive past an architecture-specific crossover (Qwen: between 3B and 7B; Gemma: between 4B and 12B), so use DPO when deployment scale is within $2\times$ of that crossover. The within-Qwen gap actually \emph{narrows} from $+6.0$~pp at 7B to $+2.2$~pp at 14B---we therefore avoid claiming a parametric form for $\Delta(N)$ and instead frame the rule by crossover region. The mechanistic basis (Section~\ref{sec:discussion}) is that \sprft{} hits a ceiling once base accuracy is high, while contrastive objectives still extract gradient from incorrect trajectories---a ceiling monotone in scale, making the dominance regime predictable even when point estimates are not.
...
\textbf{Limitations.}
We cover two model families (Qwen~2.5 up to 14B, Gemma~3 up to 12B) on mathematical reasoning; on multiple-choice general benchmarks (ARC-C, HellaSwag, WinoGrande) algorithms differ by $<$1.2~pp at all scales (Appendix~\ref{app:general_benchmarks}), but those are not generative tasks, so transfer to open-ended evaluation (AlpacaEval, MT-Bench), instruction following, safety, or creative writing remains open. Self-play data quality covaries with scale; the fixed-dataset and gold-data SFT controls (Section~\ref{sec:controls}) confirm the inversion is not a data-source artifact, but extending these controls to every scale would strengthen the claim. The 20-variant DPO sweep uses shared hyperparameters (LR, $\beta=0.1$); per-variant tuning could surface differences our plug-and-play protocol misses. Code and data release upon acceptance.
