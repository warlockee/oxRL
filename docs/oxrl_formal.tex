...
\newcommand{\justificationTODO}[1][]{\textcolor{orange}{[TODO]}#1}
...
\textbf{(1)~Rankings invert with scale}: at $\leq$3B, Self-Play RFT (\sprft{}; standard rejection sampling fine-tuning~\citep{yuan2023rft} on the model's own correct trajectories) is the strongest offline method (at 3B under per-algorithm best learning rates; Appendix~\ref{app:lr_ablation_3b}); at 7B+, \sprft{} collapses to worst while DPO leads ($p < 0.001$). The inversion replicates in Gemma~3 and persists at Qwen2.5-14B.
...
Post-training of large language models---adapting a pretrained model for tasks such as mathematical reasoning---has become the decisive stage that converts a pretrained checkpoint's raw problem-solving ability into the reliably formatted, instruction-following behavior that evaluation and deployment require~\citep{ouyang2022training, touvron2023llama2}. Many algorithms compete for adoption: online RL methods like Group Relative Policy Optimization (GRPO~\citep{shao2024deepseekmath}), offline preference methods like DPO, and a growing family of variants---SimPO, Kahneman-Tversky Optimization (KTO~\citep{ethayarajh2024kto}), Odds-Ratio Preference Optimization (ORPO~\citep{hong2024orpo})---each claiming improvements over prior work. DeepSeek-R1~\citep{deepseek2025r1} demonstrated that pure RL post-training can elicit sophisticated reasoning, further expanding the design space. Yet each paper evaluates on different models, datasets, and scales, making cross-method comparison unreliable. The few existing studies~\citep{xu2024dpo, spangher2025rlhf} cover limited methods at a single scale---they cannot detect the ranking inversions we observe.
...
\textbf{Gemma 3 (cross-architecture validation).} To test whether our findings generalize beyond Qwen, we train \sprft{}, DPO, and SimPO on Gemma~3~\citep{gemma3_2025} IT at 1B, 4B, and 12B with 3 seeds each (27 runs). This uses the same GSM8K data and evaluation protocol; 12B uses LoRA, matching the Qwen2.5-7B setup.
...
At \textbf{3B}, strict-match accuracy drops well below 1.5B---counter-intuitive but \emph{not} a genuine scaling regression: the Qwen~2.5-3B-Instruct checkpoint has a pre-existing format-compliance deficit (62.9\% flexible-match, 10.7\% strict-match) that the shared default LR ($10^{-6}$) cannot overcome. A best-LR sweep (Appendix~\ref{app:lr_ablation_3b}) confirms this is a hyperparameter pathology: \sprft{} recovers to \textbf{55.7\%}---\emph{on par with 1.5B} (54.4\%)---and \sprft{}$>$DPO holds at every LR tested. The 1.5B$\to$7B inversion claim is unaffected; 3B is a checkpoint-specific noisy interior point.
...
\midrule
\sprft{} & 33.59$_{\pm 0.26}$ \small{(\textcolor{teal}{+14.0})} & 54.36$_{\pm 0.59}$ \small{(\textcolor{teal}{+23.2})} & \textbf{18.35}$^b$ \small{(\textcolor{teal}{+7.7})} & 77.38$_{\pm 1.11}$ \small{(\textcolor{teal}{+1.6})} \\
\midrule
DPO & 33.18$_{\pm 0.31}$ \small{(\textcolor{teal}{+13.6})} & 49.08$_{\pm 0.61}$ \small{(\textcolor{teal}{+17.9})} & 14.44$_{\pm 2.01}$ \small{(\textcolor{teal}{+3.8})} & \textbf{83.38}$_{\pm 0.56}$ \small{(\textcolor{teal}{+7.6})} \\
IPO & 33.97$_{\pm 0.68}$ \small{(\textcolor{teal}{+14.4})} & 52.24$_{\pm 0.22}$ \small{(\textcolor{teal}{+21.1})} & --- & 80.39$_{\pm 0.92}$ \small{(\textcolor{teal}{+4.6})} \\
KTO & 33.38$_{\pm 0.98}$ \small{(\textcolor{teal}{+13.8})} & 51.15$_{\pm 1.77}$ \small{(\textcolor{teal}{+20.0})} & --- & 80.24$_{\pm 0.16}$ \small{(\textcolor{teal}{+4.4})} \\
SimPO & 23.55$_{\pm 5.19}$ \small{(\textcolor{teal}{+4.0})} & 38.67$_{\pm 1.78}$ \small{(\textcolor{teal}{+7.5})} & --- & 83.32$_{\pm 1.79}$ \small{(\textcolor{teal}{+7.5})} \\
\midrule
SGRPO (GRPO) & 32.45$^b$ \small{(\textcolor{teal}{+12.9})} & \textbf{58.00}$_{\pm 0.57}$ \small{(\textcolor{teal}{+26.8})} & 8.11$^c$ \small{(\textcolor{red}{$-$2.6})} & 80.59$^d$ \small{(\textcolor{teal}{+4.8})} \\
\bottomrule
\end{tabular}
\end{table}
...
Two differences from Qwen are worth noting. First, SimPO does not converge to DPO at the largest scale in Gemma, suggesting that the DPO/SimPO parity observed at Qwen2.5-7B may be architecture-specific. Second, the inversion point appears later in Gemma (12B vs.\ 7B), which may reflect differences in the pretrained checkpoints' answer-format compliance under strict-match grading (Appendix~\ref{app:format}). Despite these nuances, the central finding---that \sprft{}-to-preference rankings invert with scale---replicates across architectures.

\textbf{Qwen2.5-14B.} To extend the scaling curve beyond 12B, we train \sprft{}, DPO, and SimPO on Qwen~2.5-14B-Instruct with 3 seeds each (9 runs). The inversion persists: DPO (86.8\%~$\pm$0.23) $>$ SimPO (86.5\%~$\pm$0.68) $>$ \sprft{} (84.5\%~$\pm$0.19) $>$ Base (79.5\%). Welch's $t$-tests at $N{=}3$: DPO$>$\sprft{} significant ($p{=}0.0002$); SimPO$>$\sprft{} marginally significant ($p{=}0.030$); SimPO vs.\ DPO not detectable ($p{=}0.53$, gap $0.30$~pp $\ll$ seed noise). Within the Qwen family the DPO--\sprft{} gap \emph{narrows} from 6.0~pp at 7B to \textbf{2.2~pp} at 14B; on general benchmarks (ARC-C, HellaSwag, WinoGrande) the spread is $<$1.2~pp.
...
\begin{table}[t]
\centering
\caption{Cross-architecture validation and deployment scale on GSM8K (\%). Gemma~3: 3 seeds each; 12B uses LoRA. Qwen2.5-14B: 3 seeds each, LoRA. The \sprft{}-to-DPO inversion replicates across architectures and persists at 14B.}
\label{tab:cross_arch}
\small
\begin{tabular}{@{}lccccccc@{}}
\toprule
& \multicolumn{2}{c}{\textbf{Gemma 1B}} & \multicolumn{2}{c}{\textbf{Gemma 12B}} & \multicolumn{2}{c}{\textbf{Qwen2.5-14B}} \\
...
Tables~\ref{tab:math_results}~and~\ref{tab:math_scaling} compare GSM8K and MATH results. At 1.5B, the 19.3~pp GSM8K spread collapses to \textbf{0.54~pp} on MATH---a $36\times$ compression. Rankings shift: SGRPO drops from 1st to 4th, SimPO rises from last to 2nd. On general-domain benchmarks (ARC-C, HellaSwag, WinoGrande), the spread collapses further to \textbf{0.47~pp} ($41\times$; Appendix~\ref{app:figures}). At small scale, algorithms differ only in format compliance, not reasoning (Table~\ref{tab:math_results}; Appendix~\ref{app:format}).
...
\centering
\caption{MATH accuracy (\%) across scales. At $\geq$4B, DPO leads in Gemma but SimPO leads in Qwen2.5-14B.}
...
On harder reasoning tasks (MATH), a new dimension emerges: algorithm-architecture interaction. DPO gains +9~pp on Gemma~12B MATH while SimPO gains +13~pp on Qwen2.5-14B MATH---a complete reversal. This suggests that loss functions interact with architecture-specific representations in ways that are invisible on format-sensitive tasks like GSM8K.
...
\textbf{Why early runs were bit-identical across seeds.} In framework versions before v1.9.0, the training-data \texttt{DistributedSampler} received a fixed default seed, so offline runs differing only in \texttt{run.seed} saw identical data order; with deterministic bf16 training and greedy evaluation, they were bit-reproducible. This produced the $\sigma{=}0$ cells originally reported at 0.5B. The sampler now consumes \texttt{run.seed}; the 1.5B and 7B tables below use post-fix reruns, and Table~\ref{tab:seedfix_05b} reports a post-fix rerun of the \emph{entire} 0.5B grid.

\begin{table}[ht]
\centering
\caption{0.5B multi-seed rerun under corrected sampler seeding (GSM8K 0-shot CoT strict-match, 3 seeds each). Variance is genuine and nonzero; original point estimates were accurate to $<$1~pp; the four stable methods span 0.79~pp (below seed noise), and SimPO's $\sigma{=}5.19$ replicates its instability (Appendix~\ref{app:simpo_variance}) at a third scale.}
\label{tab:seedfix_05b}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Method} & \textbf{Mean$_{\pm\sigma}$} & \textbf{s42} & \textbf{s123} & \textbf{s456} \\
\midrule
IPO & 33.97$_{\pm 0.68}$ & 33.97 & 33.28 & 34.65 \\
\sprft{} & 33.59$_{\pm 0.26}$ & 33.43 & 33.89 & 33.43 \\
KTO & 33.38$_{\pm 0.98}$ & 34.19 & 33.66 & 32.30 \\
DPO & 33.18$_{\pm 0.31}$ & 33.36 & 33.36 & 32.83 \\
SimPO & 23.55$_{\pm 5.19}$ & 26.00 & 17.59 & 27.07 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\multirow{4}{*}{Qwen2.5-7B FT} & Base & 55.2 & 80.5 & 71.1 & 68.9 \\
& \sprft{} & 56.1 & 80.6 & 71.0 & 69.2 \\
& DPO & 55.0 & 80.6 & 70.4 & 68.7 \\
& SimPO & 54.1 & 80.6 & 69.9 & 68.2 \\
\midrule
\multirow{4}{*}{Qwen2.5-14B} & Base & 62.4 & 84.4 & 76.0 & 74.3 \\
...
\textbf{Justification:} The abstract and introduction make three claims: (1) rankings invert with scale, (2) initialization dominates algorithm choice, and (3) apparent differences have two regimes. Each is supported by a dedicated results subsection (Sec.~\ref{sec:cross_arch}, Sec.~\ref{sec:ranking_inversion}, Sec.~\ref{sec:base_ablation}, and Sec.~\ref{sec:format_compliance}), with formal statistical testing ($p<0.001$, Bonferroni-corrected) and cross-architecture replication (Gemma~3). The scope is explicitly limited to mathematical reasoning on two model families.

\item {\bf Limitations}\\
Does the paper discuss the limitations of the work performed by the authors?\\
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} A dedicated \textbf{Limitations} paragraph appears in the Discussion (page~\pageref{sec:discussion} approximately): two model families, math-only generative tasks, MC-only general benchmarks, asymmetric fixed-dataset control, and noise-floor (0.5B) / LR-pathology (3B) caveats. Appendices~\ref{app:seed_invariance} and~\ref{app:lr_ablation_3b} discuss these in detail.
...
\textbf{Justification:} The paper is an empirical study of post-training algorithms; it presents no formal theorems requiring proofs. Statistical procedures (Welch's $t$-test, Bonferroni correction) are standard and cited.

\item {\bf Experimental result reproducibility}\\
Does the paper fully disclose all the information needed to reproduce the main experimental results?\\
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} All training configurations (learning rates, batch sizes, LoRA rank, KL coefficients, optimizer, GPUs/run) are listed in Appendix~\ref{app:configs} (Table in Sec.~``Training Configurations''). Datasets (GSM8K, MATH, ARC-C, HellaSwag, WinoGrande), models (Qwen~2.5 0.5B--14B, Gemma~3 1B--12B), seed counts, and evaluation protocol are specified. The repository (oxRL) is referenced; code and data will be released upon acceptance per the Limitations statement.
...
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} All datasets used (GSM8K, MATH, ARC-C, HellaSwag, WinoGrande) are public benchmarks with standard splits. The oxRL training framework is open-source; the repository URL and pre-trained checkpoints will be deanonymized upon acceptance. Supplementary material includes training configs and evaluation scripts. Self-play training data (generated rollouts) will be released alongside checkpoints to enable exact replication.
...
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} Section~\ref{sec:methods} (``Methods and Experimental Setup'') specifies the protocol; Appendix~\ref{app:configs} lists per-algorithm hyperparameters and learning rates. Learning rate sensitivity is analyzed at 1.5B, 3B, and 7B (Appendices~\ref{app:lr_ablation_3b} and the LR-sweep appendices). Test protocols (strict-match, sampling-based) are documented in Appendix~\ref{app:sampling_eval}.
...
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} Up to $N=5$ seeds are run for headline comparisons (1.5B and 7B \sprft{}/DPO/SimPO); 3 seeds for cross-architecture (Gemma~3) and 14B runs. Standard deviations are reported throughout (e.g., $83.38\% \pm 0.56$). Welch's $t$-tests with Bonferroni correction are applied; the $\alpha=0.005$ family-wise threshold and 80\% power calculation are stated explicitly (Appendix~\ref{app:seed_invariance}, DPO variant table caption).
...
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} Appendix~\ref{app:compute} (``Compute Budget'') gives a per-category breakdown across $\sim$350 training runs: GPU type (H100 80GB), GPUs/run, time/run, and total GPU-hours per category. Grand total: $\sim$940 GPU-hours.

\item {\bf Code of ethics}\\
Does the research conducted in the paper conform, in every respect, with the NeurIPS Code of Ethics?\\
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} The work uses publicly released model weights (Qwen~2.5, Gemma~3) and public benchmark datasets (GSM8K, MATH, ARC, HellaSwag, WinoGrande) under their respective licenses. No human subjects, personal data, or sensitive deployments are involved. The study is a methodological audit of post-training algorithms.

\item {\bf Broader impacts}\\
Does the paper discuss both potential positive societal impacts and negative societal impacts of the work performed?\\
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} The Discussion notes that the central finding---small-scale algorithm rankings invert at deployment scale---has direct positive impact for the post-training research community by exposing a methodological hazard and arguing for deployment-scale validation. Negative impacts are minimal: the paper releases no new generative capability beyond what is already publicly available; it studies algorithm-selection methodology, not new model artifacts.

\item {\bf Safeguards}\\
Does the paper describe safeguards that have been put in place for responsible release of data or models with a high risk for misuse (e.g., pretrained language models, image generators, or scraped datasets)?\\
\textbf{Answer:} \answerNA{}\\
\textbf{Justification:} The paper releases fine-tuned math-reasoning checkpoints derived from already-public base models (Qwen~2.5, Gemma~3). Math-task fine-tunes do not introduce new misuse risks beyond those of the base models, which already shipped with their own safety releases.

\item {\bf Licenses for existing assets}\\
Are the creators or original owners of assets (e.g., code, data, models) used in the paper properly credited and are the license and terms of use explicitly mentioned and respected?\\
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} All base models (Qwen~2.5~\citep{qwen2025qwen25}, Gemma~3~\citep{gemma3_2025}) and benchmarks (GSM8K, MATH, ARC, HellaSwag, WinoGrande) are cited. Qwen~2.5 is released under Apache~2.0 (0.5B/1.5B/7B/14B) and Qwen-Research license (3B); Gemma~3 is under the Gemma Terms of Use; the benchmarks are MIT/Apache~2.0/CC-BY. All uses are within license terms (research, non-commercial evaluation).

\item {\bf New assets}\\
Are new assets introduced in the paper well documented and is the documentation provided alongside the assets?\\
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} The oxRL framework (release planned upon deanonymization) ships with a README documenting training, evaluation, and reproduction commands; a Croissant metadata file documents released self-play datasets including biases, collection, and PII fields. Per-checkpoint model cards specify training algorithm, base model, hyperparameters, and intended use.
...
\textbf{Answer:} \answerNA{}\\
\textbf{Justification:} No crowdsourcing or human-subject experiments. All evaluation uses automated metric extraction on public benchmarks.
...
\textbf{Answer:} \answerNA{}\\
\textbf{Justification:} No human-subject research; IRB approval not applicable.

\item {\bf Declaration of LLM usage}\\
Does the paper describe the usage of LLMs if it is an important, original, or non-standard component of the core methods in this research?\\
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} LLMs are the object of study: the paper post-trains and evaluates open-weight LLMs (Qwen~2.5 0.5B--14B, Gemma~3 1B--12B). Their roles---as base policies, as self-play trajectory generators (\sprft{}), and as evaluation targets---are documented in Sec.~\ref{sec:methods} and Appendix~\ref{app:configs}. LLMs were not used as part of the writing methodology beyond standard editing assistance.

\end{enumerate}

\section{Discussion-Period Additions}
\label{app:discussion_additions}

The following experiments were run during the NeurIPS discussion period in response to reviewer requests. Raw evaluation JSONs accompany the released artifacts.

\subsection{Qwen2.5-32B Deployment Scale}
\label{app:qwen32b}

Identical protocol to the 14B deployment runs: self-play data generated by the 32B model itself, shared hyperparameters, GSM8K 8-shot strict-match, greedy. Of 7,473 prompts, only \textbf{816} yield informative preference pairs at 32B (6,657 discarded because all 16 samples agree), versus 1,332 at 14B --- a direct measurement of the RFT ceiling closing.

\begin{table}[ht]
\centering
\caption{Qwen2.5-32B on GSM8K (8-shot strict-match; mean $\pm\sigma$ over 3 seeds). The small-scale ranking is fully reversed: SimPO (worst at $\leq$1.5B) is best; \sprft{} (best at $\leq$3B) gains $+0.30\pm0.23$~pp over base --- statistically zero. \sprft{}'s gain over base collapses $+23.2$~pp (1.5B) $\to$ $+1.6$ (7B) $\to$ $+0.3$ (32B); DPO $>$ \sprft{} at $p<0.01$ (Welch).}
\label{tab:qwen32b}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Method} & \textbf{GSM8K (\%)} & \textbf{$\Delta$ vs.\ Base} \\
\midrule
Base (Instruct) & 80.52 & --- \\
\sprft{} & 80.82$_{\pm 0.23}$ & +0.30 \\
DPO & 82.23$_{\pm 0.04}$ & +1.71 \\
SimPO & \textbf{83.42}$_{\pm 1.14}$ & \textbf{+2.90} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Fixed-Dataset Control on Gemma 3}
\label{app:gemma_fixed_data}

Gemma-3-1B trained on Gemma-3-12B-generated self-play data (3 seeds, identical hyperparameters); own-data checkpoints re-evaluated under the identical protocol (GSM8K 8-shot strict-match, vLLM backend). Unlike Qwen --- where 14B-quality data preserves the small-scale \sprft{}$>$DPO ordering (Section~\ref{sec:controls}) --- strong-teacher data \emph{flips} the ordering at Gemma-1B, consistent with a capability-gap effect: \sprft{} imitates trajectories the 1B model cannot reproduce, while DPO's contrastive signal degrades gracefully. The fixed-dataset conclusion of Section~\ref{sec:controls} is therefore scoped to Qwen; across families, rankings are unstable across scale, architecture, \emph{and data provenance}.

\begin{table}[ht]
\centering
\caption{Gemma-3-1B fixed-dataset control (GSM8K 8-shot strict-match, 3 seeds).}
\label{tab:gemma_fixed}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Training data} & \textbf{\sprft{}} & \textbf{DPO} & \textbf{Winner} \\
\midrule
Own (1B) self-play & \textbf{26.91}$_{\pm 0.55}$ & 22.82$_{\pm 2.08}$ & \sprft{} (+4.1~pp) \\
12B-generated & 21.41$_{\pm 1.01}$ & \textbf{24.69}$_{\pm 1.01}$ & DPO (+3.3~pp) \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Previously-Missing 3B Cells}
\label{app:3b_fill}

IPO, KTO, and SimPO at 3B under the shared default LR ($10^{-6}$), 3 seeds each (GSM8K 8-shot strict-match): IPO $24.92_{\pm 0.85}$, KTO $23.68_{\pm 0.87}$, SimPO $9.60_{\pm 3.84}$. All methods remain far below the base checkpoint's 62.9\% flexible-extract ability, confirming the 3B default-LR pathology (Section~\ref{sec:ranking_inversion}) is checkpoint-wide rather than method-specific, with SimPO again the most fragile.
