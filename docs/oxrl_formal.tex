...
We frame the question as a \emph{scaling-law study of post-training algorithms}---measuring how algorithm rankings evolve as a function of model size---and show this practice is \emph{anti-predictive} for mathematical reasoning: rankings invert as models grow.
...
\textbf{(1)~Rankings invert with scale}: at $\leq$1.5B, Self-Play RFT (\sprft{}) is the strongest offline method; at 7B+, \sprft{} collapses to worst while DPO leads ($p < 0.001$). The inversion replicates in Gemma~3 and persists at Qwen~14B.
...
\begin{figure}[h]
\centering
\begin{tikzpicture}[x=1.05cm, y=0.065cm]
  % axes — y origin at GSM8K=20 so SimPO 0.5B (=26.08) is on-chart
  \draw[->, thick] (-0.25, 0) -- (5.7, 0) node[right, font=\small] {model size};
  \draw[->, thick] (-0.25, 0) -- (-0.25, 78) node[above, font=\small] {GSM8K \%};
  % y ticks (axis y = GSM8K \% - 20)
  \foreach \y/\lab in {0/20, 10/30, 30/50, 50/70, 70/90} {
  ...
  \foreach \x/\lab in {0/0.5B, 1.3/1.5B, 2.1/3B, 3.6/7B, 4.8/14B} {
    \draw (\x, -1.2) -- (\x, 1.2) node[below=2pt, font=\scriptsize] {\lab};
  }
  % SP-RFT (orange-ish): 33.97, 54.36, 55.70, 77.38, 84.5  (axis y = val - 20)
  \draw[orange!85!black, thick]
    (0, 13.97) -- (1.3, 34.36) -- (2.1, 35.70) -- (3.6, 57.38) -- (4.8, 64.5);
  \foreach \p in {(0,13.97),(1.3,34.36),(2.1,35.70),(3.6,57.38),(4.8,64.5)}
    \fill[orange!85!black] \p circle (0.06cm);
  \node[orange!85!black, font=\scriptsize] at (1.5, 40) {SP-RFT};
  % DPO (blue): 33.97, 49.08, 34.55, 83.38, 86.78
  \draw[blue!75!black, thick, dashed]
    (0, 13.97) -- (1.3, 29.08) -- (2.1, 14.55) -- (3.6, 63.38) -- (4.8, 66.78);
  \foreach \p in {(0,13.97),(1.3,29.08),(2.1,14.55),(3.6,63.38),(4.8,66.78)}
    \fill[blue!75!black] \p circle (0.06cm);
  \node[blue!75!black, font=\scriptsize] at (3.0, 22) {DPO};
  % SimPO (teal): 26.08, 38.67, --, 83.32, 86.48 (skip 3B with dashed gap)
  \draw[teal!80!black, thick, dotted]
    (0, 6.08) -- (1.3, 18.67) -- (3.6, 63.32) -- (4.8, 66.48);
  \foreach \p in {(0,6.08),(1.3,18.67),(3.6,63.32),(4.8,66.48)}
...
\end{tikzpicture}
\caption{\textbf{A scaling-law view of post-training algorithms.} GSM8K accuracy vs.\ model size (log axis) for three offline post-training methods on Qwen~2.5. Algorithm \emph{rankings} cross between 3B and 7B: \sprft{} is best at $\leq$3B but worst at $\geq$7B, while DPO and SimPO catch up and overtake. Selecting an algorithm at 1.5B is \emph{anti-predictive} of the choice at deployment scale. The inversion replicates on Gemma~3 (1B--12B; Section~\ref{sec:cross_arch}).}
\label{fig:summary}
\end{figure}
...
Suppose you are choosing a post-training algorithm for mathematical reasoning. You benchmark Self-Play RFT (\sprft{}), DPO, and SimPO at 1.5B parameters: \sprft{} wins by 5~pp over DPO. You select \sprft{}, scale to 7B, and deploy---only to discover that \sprft{} is now the \emph{worst} method by 6~pp, while DPO has silently become best. Your small-scale selection was not just uninformative; it was anti-predictive (Figure~\ref{fig:summary}).

\textbf{We frame this as a scaling-law study of post-training algorithms}: rather than chase a single best loss function, we measure how algorithm rankings evolve as a function of model size, treating the curves themselves---not the point estimates---as the object of study.
...
\begin{table}[t]
\centering
\caption{GSM8K exact-match accuracy (\%) across scales (Qwen 2.5). 1.5B: 3-seed means $\pm \sigma$; 7B: 5-seed means for \sprft{}/DPO/SimPO, 3-seed for IPO/KTO. Best per scale in \textbf{bold}. $^b$Single seed. $^c$Epoch-2 checkpoint. $^d$Best of 3 epochs. $^a$62.9\% flexible but 10.69\% strict-match. The 3B column reports best-LR results from the dedicated 3B sweep (Appendix~\ref{app:lr_ablation_3b}); at the shared default LR $10^{-6}$, the 3B numbers are \sprft{} 18.35, DPO 14.44, SimPO 6.90, included so the column is comparable to the rest of the table; the \sprft{}$>$DPO ordering at 3B holds at \emph{every} LR tested.}
...
\textbf{Gold-data SFT control.} Because \sprft{} trains on self-play trajectories (functionally RFT), a natural objection is that the ranking inversion merely reflects RFT's ceiling effect at scale, not a property of SFT \emph{per se}. To test this, we train standard SFT on MetaMathQA~\citep{yu2024metamath}---a high-quality, externally generated dataset of 240K GSM8K-related problems with expert solutions (3 seeds each at both 1.5B and 7B).
...
For mathematical reasoning, the dominant paradigm of small-scale benchmarking + extrapolation is unreliable. Practitioners should validate at deployment scale; the research community should invest in how algorithms interact with scale, initialization, and task structure, rather than designing new loss functions at a single small scale. Whether this extends to other post-training objectives (instruction following, safety) is open---our MC-only general-benchmark control limits direct extrapolation to generative tasks.

\textbf{Limitations.}
Two model families (Qwen~2.5 14B, Gemma~3 12B) on mathematical reasoning; on multiple-choice general benchmarks the spread is $<$1.2~pp at all scales (Appendix~\ref{app:general_benchmarks}) but those are not generative, so transfer to AlpacaEval/MT-Bench, instruction following, safety, or creative writing remains open. Self-play data quality covaries with scale; the controls in Section~\ref{sec:controls} confirm the inversion is not a data-source artifact, but extending them to every scale would strengthen the claim. The 20-variant DPO sweep uses shared $\beta{=}0.1$, LR$=10^{-6}$; per-variant tuning could surface differences our plug-and-play protocol misses. Code, configs and per-seed JSONs release upon acceptance.
