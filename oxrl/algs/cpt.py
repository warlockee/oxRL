...
class CPT:
    ...
    def forward(self, batch):
        '''
            ...
            Unlike SFT, CPT trains on ALL non-padding tokens (prompt + response)
            by using attn_mask instead of loss_mask.
        '''
        ...
        # CPT difference: train on ALL non-padding tokens (prompt + response).
        # Use attn_mask[:, 1:] instead of batch['loss_mask'] so every
        # non-padding token contributes to the loss.
        loss_mask = batch['attn_mask'][:, 1:].contiguous()
