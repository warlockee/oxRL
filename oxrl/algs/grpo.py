...
class GRPO(BaseAlgorithm):
    ...
    def offload_to_cpu(self) -> bool:
        ...
        This frees GPU memory so the vLLM rollout engine (a separate Ray actor
        sharing the same physical GPU) can reload the updated model weights.

        With ZeRO-3, each rank holds a partition of optimizer states and
        parameters. We:
        1. Zero out gradients (they're not needed between epochs)
        2. Move optimizer state tensors to CPU
        3. Flush the CUDA cache

        The model parameters stay on GPU (ZeRO-3 manages them), but freeing
        optimizer states alone recovers ~2x the parameter memory (Adam has 2
        ...
        try:
            # 1. Zero gradients to free gradient memory
            self.policy_engine.zero_grad()

            # 2. Offload optimizer state tensors to CPU
            optimizer = self.policy_engine.optimizer
            offloaded = 0
            # DeepSpeed wraps the optimizer; access the base optimizer's state
            ...
            # 3. Synchronize and free cached memory
    ...
    def reload_to_gpu(self) -> bool:
        ...
        Moves all optimizer state tensors that were offloaded to CPU back to the
        training device (GPU).
        """
        rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
        device = self.policy_engine.device
